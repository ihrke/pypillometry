...
`pypillometry` is a Python package for analyzing pupillometric data, providing a comprehensive toolkit for preprocessing, analyzing, and visualizing pupil size measurements and eye-tracking data. The package is built around the :class:`EyeData`, :class:`PupilData`, and :class:`GazeData` classes, which serves as the main interface for handling pupillometric/eyetracking datasets. 
...
So far, the package provides functions to load data from Eyelink's .edf files using the `eyelinkio <https://github.com/scott-huberty/eyelinkio>`_ package via the function :func:`GenericEyeData.from_eyelink`. This function will import the raw data stored in the EDF file as well as all "message" triggers. It is also possible to use Python's excellent functionality to parse text-based datafiles (most eyetrackers allow to export their proprietory format into a text-based one). For example, Eyelink's `ASC-format <http://download.sr-support.com/dispdoc/page25.html>`_ generated by the EDF2ASC conversion tool outputs space-separated data that can be easily loaded using the `I/O functionality of the pandas package <https://pandas.pydata.org/pandas-docs/stable/reference/io.html>`_ . 
Once the data has been manually parsed into :mod:`numpy`-arrays, it can be converted into a :class:`PupilData`, :class:`GazeData`, or :class:`EyeData` object.


.. autosummary:: 
    ~pypillometry.GenericEyeData.from_eyelink    
    ~pypillometry.io.load_study_osf
    ~pypillometry.io.load_study_local
    ~pypillometry.io.osf_authenticate
    ~pypillometry.example_datasets.get_example_data

:mod:`pypillometry` provides functions to load data from local data directories (:func:`load_study_local`) and from OSF-projects (:func:`load_study_osf`) based on a user-provided configuration file (:ref:`/examples/pypillometry_conf.py`).

The package comes with a few example datasets for convenience (see :ref:`this notebook </example_datasets.ipynb>`). These can be loaded using the :func:`get_example_data()` function.
...
The following is a list of functions designed for detecting artifacts. They all operate by detecting segments of suspicious data and either masking them in the data itself or returning them as :class:`Intervals` objects for inspection. 

.. autosummary::

    PupilData.pupil_blinks_detect
    GenericEyeData.blinks_merge_close
    GazeData.mask_eye_divergences
    GazeData.mask_offscreen_coords
...
- `apply_mask=`: if `True`, the detected artifacts are applied as masks to the data and the object is returned; if `False`, the detected artifacts are returned as :class:`Intervals` objects. 
...
Assuming you have generated a :class:`~pypillometry.PupilData` object, a range of pre-processing functions are available. The main pre-processing issues with pupillometric data are:
