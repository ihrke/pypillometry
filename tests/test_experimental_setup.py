...
class TestExperimentalSetupInitialization:
    ...
    def test_eye_to_screen_center(self):
        """Test initialization with eye-to-screen distance."""
        setup = ExperimentalSetup(eye_to_screen_center="70 cm")
...
class TestExperimentalSetupEyePosition:
    ...
    def test_eye_to_screen_center_calculation(self):
        """Test eye_to_screen_center property returns correct value."""
        setup = ExperimentalSetup(
            eye_to_screen_center="700 mm",
            screen_offset=("30 mm", "40 mm")
        )
        # eye_to_screen_center = 700 (input)
        # d = sqrt(700^2 - 30^2 - 40^2) ≈ 698.21 (computed)
        # eye_to_screen_center property = sqrt(d^2 + 30^2 + 40^2) = 700 (back to input)
        assert np.isclose(setup.eye_to_screen_center, 700.0)
    
    def test_default_screen_offset_is_zero(self):
        """Test that default eye offset is (0, 0)."""
        setup = ExperimentalSetup(eye_to_screen_center="700 mm")
...
class TestExperimentalSetupUnitParsing:
    ...
    def test_distance_m(self):
        ...
        setup = ExperimentalSetup(eye_to_screen_center="0.7 m")
        assert setup.d == 700.0
    ...
    def test_pint_quantity_distance(self):
        ...
        setup = ExperimentalSetup(eye_to_screen_center=70 * pp.ureg.cm)
        assert setup.d == 700.0
